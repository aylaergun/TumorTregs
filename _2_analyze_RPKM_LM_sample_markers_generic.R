...
RSEM_Gene_IlluminaHiSeq<-read.table(paste(fdir,"/CancerType.RSEM_genes_normalized.txt",sep=""),header=TRUE,sep="\t");
...
TCell_index=matrix(nrow=10,ncol=length(tumor_id));
BCell_index=matrix(nrow=10,ncol=length(tumor_id));
DC_index=matrix(nrow=10,ncol=length(tumor_id));
Eos_index=matrix(nrow=10,ncol=length(tumor_id));
MPhage_index=matrix(nrow=10,ncol=length(tumor_id));
Neutrophil_index=matrix(nrow=10,ncol=length(tumor_id));
NK_index=matrix(nrow=10,ncol=length(tumor_id));
Mast_index=matrix(nrow=10,ncol=length(tumor_id));

#Calculate average gene expression among signature genes in tumor samples by randomly selecting 80% of the marker genes 10 times
...
keep1=sample(c(1:length(keep)),4*round(length(keep)/5),replace=FALSE);
TCell_index[k,]=colMeans(RPKM_log2[TCell_Genes_id[keep[keep1]],tumor_id]);
...
keep1=sample(c(1:length(keep)),4*round(length(keep)/5),replace=FALSE);
BCell_index[k,]=colMeans(RPKM_log2[BCell_Genes_id[keep[keep1]],tumor_id]);
...
keep1=sample(c(1:length(keep)),4*round(length(keep1)/5),replace=FALSE);
DC_index[k,]=colMeans(RPKM_log2[DC_Genes_id[keep[keep1]],tumor_id]);

Eos_Genes=intersect(unlist(Eos_Cell_Sign),names);
Eos_Genes_id=match(Eos_Genes,names);
keep=which(apply(RPKM_log2[Eos_Genes_id,tumor_uid],1,mean)>1);
keep1=sample(c(1:length(keep)),4*round(length(keep)/5),replace=FALSE);
Eos_index[k,]=colMeans(RPKM_log2[Eos_Genes_id[keep[keep1]],tumor_id]);
...
keep1=sample(c(1:length(keep)),4*round(length(keep)/5),replace=FALSE);
MPhage_index[k,]=colMeans(RPKM_log2[MPhage_Genes_id[keep[keep1]],tumor_id]);
...
keep1=sample(c(1:length(keep)),4*round(length(keep)/5),replace=FALSE);
Neutrophil_index[k,]=colMeans(RPKM_log2[Neutrophil_Genes_id[keep[keep1]],tumor_id]);
...
keep1=sample(c(1:length(keep)),4*round(length(keep)/5),replace=FALSE);
NK_index[k,]=colMeans(RPKM_log2[NK_Genes_id[keep[keep1]],tumor_id]);
...
keep1=sample(c(1:length(keep)),4*round(length(keep)/5),replace=FALSE);
Mast_index[k,]=colMeans(RPKM_log2[Mast_Genes_id[keep[keep1]],tumor_id]);
}

for (k in 1:10)
{

A1=rbind(RPKM_log2[,tumor_id],TCell_index[k,],BCell_index[k,],DC_index[k,],Eos_index[k,],MPhage_index[k,],Neutrophil_index[k,],NK_index[k,],Mast_index[k,]);
rownames(A1)=c(rownames(RPKM_log2),"TCell_index","BCell_index","DC_index","Eos_index","MPhage_index","Neutrophil_index","NK_index","Mast_index");
A2=as.data.frame(t(A1));


Residuals_Individual<-matrix(nrow=20502,ncol=length(tumor_id));
...
New_RPKM=log2(New_RPKM1[,tumor_id]+1);
...
colnames(Residuals_Individual)=colnames(RPKM[,tumor_id]);
save("New_RPKM","Residuals_Individual",file=paste("CancerType_Residuals_Log2_Jan2018_sample_markers_v",k,".RData",sep=""));
}
